...
@router.get("/111")
async def test(request: Request, db: Session = Depends(get_db),msg:str = None, current_user:User = Depends(get_current_user_from_token)):   #new
    return 'test'

@router.get("/")
async def home(request: Request, db: Session = Depends(get_db),msg:str = None, current_user:User = Depends(get_current_user_from_token)):   #new
...
def show_jobs_to_delete(request: Request, db: Session = Depends(get_db), current_user:User = Depends(get_current_user_from_token)):
    jobs = list_jobs(db=db)
    return templates.TemplateResponse(
